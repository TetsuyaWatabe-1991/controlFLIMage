# -*- coding: utf-8 -*-
"""
Created on Wed Dec 21 09:50:51 2022
...
class control_flimage():
    ...
    def __init__(self):
        ...
        flipres =self.get_val_sendCommand('State.Acq.flipXYScan')
        self.xyflip= [] 
        for TF in [val for val in flipres.strip('][').split(', ')] :
            if TF=="True":
                self.xyflip.append(-1)
            elif TF=="False":
                self.xyflip.append(+1)
    ...
    def go_to_relative_pos(self):
        ...
        z_str=str(z+self.relative_zyx[0])
    ...
    def get_relative_zyx(self, shifts_zyx_pixel):
        # These + and - are dependent of the direction, so might be different in each stage.
    ...
    def align_two_flimfile(self):
        filelist=[self.flimlist[0],self.flimlist[-1]]
        print(filelist)
        ...
        self.get_relative_zyx(self.shifts_zyx_pixel)
    ...
    def wait_until_next(self,each_acquisition_from,sleep_every_sec=0.01):
        for i in range(int(self.interval_sec/sleep_every_sec)):
            each_acquisition_len=(datetime.now()-each_acquisition_from).seconds
            if each_acquisition_len>=self.interval_sec:
                break
            sleep(sleep_every_sec)
    ...
    def analyze_uncaging_point_TYX(self,threshold_coordinate=0.5,Gaussian_pixel=3):
        ...
        if self.binary_include_dendrite.max() == 0:
            ...
            self.props=regions[0]
            self.binarized=th3>0
            self.find_best_point(TwoD=True)
            
        
    def find_best_point(self,TwoD=False):
    ...
    def go_to_uncaging_plane(self):
        ...
        self.relative_zyx = [z_move_um + z_relative,0,0]
        self.go_to_relative_pos()
    ...
    def back_to_stack_plane(self):
        self.relative_zyx = [-self.uncaging_relativeZ_moved,0,0]
        self.go_to_relative_pos()
    ...
    def drift_cont_single_plane(self,xy_stage_move=True,
        ...
        if xy_stage_move==False:
            self.get_relative_zyx([[0,0,0],[temporary_shift_z,0,0]])
        else:
            self.get_relative_zyx([[0,0,0],[temporary_shift_z,
                                    self.single_shift[0],
                                    self.single_shift[1]]])

        self.append_drift_list()
        self.go_to_relative_pos()
        self.get_relative_zyx([[0,0,0],[shift_z,
                                self.single_shift[0],
                                self.single_shift[1]]])        
    ...
    def start_repeat(self,single_plane_align=True):
        ...
        for NthAc in range(self.RepeatNum):
            ...
            # if self.DoUncaging==True:
            #     self.flim.sendCommand(f"StartUncaging, {self.uncaging_x}, {self.uncaging_y}") 
