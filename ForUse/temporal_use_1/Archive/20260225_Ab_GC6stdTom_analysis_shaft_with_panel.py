...
drug_addition_datetime_str = "2026-02-25T15:05:00.000"
acquisiton_start_datetime_str = "2026-02-25T13:20:00.000"
...
fulltimeseries_df["dt"] = pd.to_datetime(fulltimeseries_df["acq_time_str"])

#add header to "group", if the dt is before the 
drug_addition_datetime = datetime.datetime.strptime(drug_addition_datetime_str, "%Y-%m-%dT%H:%M:%S.%f")
...
for each_group in fulltimeseries_df["group"].unique():
    for each_filepath in fulltimeseries_df[fulltimeseries_df["group"] == each_group]["file_path"].unique():
        each_filepath_df = fulltimeseries_df[fulltimeseries_df["file_path"] == each_filepath]
        if each_filepath_df["dt"].min() < drug_addition_datetime:
            fulltimeseries_df.loc[each_filepath_df.index, "group"] = f"pre_{each_group}"
        else:
            fulltimeseries_df.loc[each_filepath_df.index, "group"] = f"post_{each_group}"
        ...
        for each_ROI_name in ROI_name_list:
            for each_ch in ['Ch1', 'Ch2']:
                ...
                for each_signal in ['lifetime', 'intensity']:
                    ...
                    representative_transient_signal = each_transient_signal_df.iloc[first_uncaging_frame:first_uncaging_frame+4].mean()
...
plot_info_dict = {
    "GCaMP_transient_Spine_intensity": {"ylabel": r"GCaMP F/F0", 
                            ...
                            "ylim": [fulltimeseries_df["transient_Spine_Ch1_intensity"].min()-0.1, 
                                     fulltimeseries_df["transient_Spine_Ch1_intensity"].max()+0.1],
                            },
...
for each_header, each_header_name in group_header_dict.items():
    ...
    for each_uncaging_power_coherent_mW in fulltimeseries_df["uncaging_power_coherent_mW"].unique():
        ...
        for each_plot_type, each_plot_info in plot_info_dict.items():
            ...
            plt.plot([0, 0.1], [ninty_percent_ylim, ninty_percent_ylim], "k-")
    ...
    for each_plot_type, each_plot_info in plot_info_dict.items():
        ...
        for col_idx, (each_header, each_header_name) in enumerate(valid_group_headers):
            ...
            for row_idx, each_uncaging_power_coherent_mW in enumerate(sorted_uncaging_powers):
                ...
                ylim = ax.get_ylim()
                ninty_percent_ylim = ylim[0] + (ylim[1] - ylim[0]) * 0.9
                ax.plot([0, 2.048 * 0.5], [ninty_percent_ylim, ninty_percent_ylim], "k-")
