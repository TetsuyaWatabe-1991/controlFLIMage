# %%
"""
Based on 20260724_GFP_LTPanalysis.py, with lifetime time-series / swarm /
combined-stats plots enabled (in addition to intensity).

Outputs go to .../summary_with_lifetime/ so intensity-only figures are not overwritten.
"""
import json
import os
import sys
...
group_header_dict = {
    ...
    "actinin_cnt": "Control",
    "actinin_tension": "Actinin tension",
}

group_header_combined = build_group_header_combined(group_header_dict)


acquisiton_start_datetime_str = "2026-07-22T11:40:00.000"
# df_save_path_1 = r"G:/ImagingData/Tetsuya/20260515/auto1\combined_df_1.pkl"
df_save_path_1 = r"G:\ImagingData\Tetsuya\20260722\auto2\combined_df_respan.pkl"
out_csv_path = r"G:\ImagingData\Tetsuya\20260722\auto2\combined_df_respan_intensity_lifetime_all_frames.csv"
...
photon_threshold_for_lifetime = 500
...
save_folder = os.path.join(os.path.dirname(df_save_path_1), "summary_with_lifetime")
...
lifetime_abs_col = f"Spine_Ch{ch_1or2}_lifetime"
lifetime_delta_ts_col = f"Spine_Ch{ch_1or2}_lifetime_normalized"
lifetime_abs_vals = fulltimeseries_df[lifetime_abs_col].dropna()
lifetime_delta_vals = fulltimeseries_df[lifetime_delta_ts_col].dropna()
lifetime_abs_ylim = [
    float(lifetime_abs_vals.min()) - 0.05 if len(lifetime_abs_vals) else 1.5,
    float(lifetime_abs_vals.max()) + 0.05 if len(lifetime_abs_vals) else 3.0,
]
lifetime_delta_ylim = [
    float(lifetime_delta_vals.min()) - 0.05 if len(lifetime_delta_vals) else -0.3,
    float(lifetime_delta_vals.max()) + 0.05 if len(lifetime_delta_vals) else 0.3,
]
plot_info_dict = {
    # Absolute lifetime (ns): no pre-normalization
    "lifetime_absolute": {
        "ylabel": "Lifetime (ns)",
        "xlabel": "Time (sec)",
        "x": "aligned_time_sec",
        "y": lifetime_abs_col,
        "errorbar": "se",
        "ylim": lifetime_abs_ylim,
        "plot_zero_line": False,
    },
    # Relative lifetime: pre-phase mean subtracted (not divided)
    "lifetime_delta": {
        "ylabel": r"$\Delta$lifetime (ns)",
        "xlabel": "Time (sec)",
        "x": "aligned_time_sec",
        "y": lifetime_delta_ts_col,
        "errorbar": "se",
        "ylim": lifetime_delta_ylim,
        "plot_zero_line": True,
    },
...
lifetime_delta_col = f"delta_lifetime_ch{ch_1or2}"
lifetime_swarm_vals = summary_df[lifetime_delta_col].dropna()
if len(lifetime_swarm_vals):
    lt_span = float(lifetime_swarm_vals.max() - lifetime_swarm_vals.min())
    lt_pad = max(lt_span * 0.1, 0.05)
    lifetime_swarm_ylim = [
        float(lifetime_swarm_vals.min()) - lt_pad,
        float(lifetime_swarm_vals.max()) + lt_pad,
    ]
else:
    lifetime_swarm_ylim = [-0.2, 0.2]
plot_info_dict = {
    "lifetime": {
        "ylabel": r"$\Delta$lifetime (ns)",
        "y": lifetime_delta_col,
        "ylim": lifetime_swarm_ylim,
    },
    "intensity": {
...
# %% combined swarmplot (same style as panel_*_swarmplot) + Welch p-value
...
valid_group_headers_combined = []
for each_header_name, header_list in group_header_combined.items():
    if not df_matches_group_headers(summary_df, header_list).empty:
        valid_group_headers_combined.append((header_list, each_header_name))

sorted_powers_combined = sorted(summary_df["uncaging_power_coherent_mW"].dropna().unique())

if len(valid_group_headers_combined) >= 2 and len(sorted_powers_combined) > 0:
    for metric in combined_metric_specs:
        y_col = metric["y_col"]
        ylabel = metric["ylabel"]
        file_tag = metric["file_tag"]
        if y_col not in summary_df.columns:
            print(f"Skip combined stats for {file_tag}: missing column {y_col}")
            continue

        y_all = summary_df[y_col].dropna()
        if y_all.empty:
            continue
        y_span = float(y_all.max() - y_all.min())
        y_pad = max(y_span * 0.1, 0.05)
        y_lim = [float(y_all.min()) - y_pad, float(y_all.max()) + y_pad]

        for each_power in sorted_powers_combined:
            n_cols = len(valid_group_headers_combined)
            fig, axes = plt.subplots(
                1,
                n_cols,
            ...
            group_vals_for_test = []
            group_names_for_test = []
            for col_idx, (header_list, each_header_name) in enumerate(valid_group_headers_combined):
                ax = axes[col_idx]
                each_header_summary_df = df_matches_group_headers(summary_df, header_list)
                plot_df = each_header_summary_df[
                    each_header_summary_df["uncaging_power_coherent_mW"] == each_power
                ]
                if plot_df.empty or plot_df[y_col].dropna().empty:
                    ax.axis("off")
                    continue

                vals = plot_df[y_col].dropna().to_numpy(dtype=float)
                group_vals_for_test.append(vals)
                group_names_for_test.append(each_header_name)
            ...
            stats_note = ""
            if len(group_vals_for_test) >= 2:
                welch = ttest_ind(
                    group_vals_for_test[0],
                    group_vals_for_test[1],
                    equal_var=False,
                    nan_policy="omit",
                )
                ...
                stats_note = (
                    f"Welch p={welch.pvalue:.3g} ({star}); "
                    f"Mann-Whitney p={mw.pvalue:.3g}"
                )
                fig.suptitle(stats_note, fontsize=9, y=1.02)
                stats_csv = os.path.join(
                    save_folder,
                    f"panel_{file_tag}_swarmplot_combined_stats_{each_power:g}mW.csv",
                )
                pd.DataFrame(
                    [
                        {
                            ...
                            "group_a": group_names_for_test[0],
                            "group_b": group_names_for_test[1],
                            "n_a": len(group_vals_for_test[0]),
                            "n_b": len(group_vals_for_test[1]),
                            "mean_a": float(np.mean(group_vals_for_test[0])),
                            "std_a": float(np.std(group_vals_for_test[0], ddof=1))
                            if len(group_vals_for_test[0]) > 1
                            else np.nan,
                            "mean_b": float(np.mean(group_vals_for_test[1])),
                            "std_b": float(np.std(group_vals_for_test[1], ddof=1))
                            if len(group_vals_for_test[1]) > 1
                            else np.nan,
            ...
            savepath = os.path.join(
                save_folder,
                f"panel_{file_tag}_swarmplot_combined_stats_{each_power:g}mW.png",
            )
            plt.savefig(savepath, dpi=150, bbox_inches="tight")
            print(f"Saved combined stats swarmplot: {savepath}")
            plt.show()
else:
    print("Skip combined stats swarmplot (need >=2 groups).")
...
def _pad_lim(series: pd.Series, pad_frac: float = 0.1) -> list[float]:
    vals = series.dropna()
    if len(vals) == 0:
        return [-0.1, 0.1]
    lo = float(vals.min())
...
plot_info_dict = {
    ...
    f"delta_lifetime_ch{ch_1or2}_vs_delta_FF0_ch{ch_1or2}":
                            {
                                "ylabel": rf"$\Delta$lifetime Ch{ch_1or2} (ns)",
                                "xlabel": rf"$\Delta$spine volume Ch{ch_1or2} (a.u.)",
                                "y": f"delta_lifetime_ch{ch_1or2}",
                                "x": f"delta_FF0_intensity_ch{ch_1or2}",
                                "ylim": _pad_lim(summary_df[f"delta_lifetime_ch{ch_1or2}"]),
                                "xlim": _pad_lim(summary_df[f"delta_FF0_intensity_ch{ch_1or2}"]),
                                "plot_zero_lines": True,
                            },
                }
