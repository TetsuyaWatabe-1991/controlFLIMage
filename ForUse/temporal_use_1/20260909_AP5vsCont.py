...
# LTP mean±SEM uses binned_min (example_gui_usage / everymin_normalize).
BINNED_TIME_XLIM_MIN = -10.0
BINNED_TIME_XLIM_MAX = 35.0
BIN_TIME_THRESHOLD_SEC = 5.0
BIN_PERCENT_MEDIAN = 0.99


CONDITION_PREFIX_ORDER = ["Control", "AP5"]


def condition_from_label(label) -> str:
    """Assign AP5 vs Control from a path or group name starting with AP5_ or cnt_."""
    text = str(label).replace("\\", "/")
    candidates = [part for part in text.split("/") if part]
    for name in reversed(candidates):
        if name.startswith("AP5_"):
            return "AP5"
...
def assign_condition(file_path=None, group=None) -> str:
    """Resolve condition from file_path first, then group label."""
    for value in (file_path, group):
        if value is None:
            continue
        cond = condition_from_label(value)
        if cond != "unknown":
            return cond
    return "unknown"


def build_condition_group_specs(df):
    """Return [(label, condition), ...] for AP5_ vs cnt_ filename prefixes."""
    present = {str(x) for x in df["condition"].dropna().unique()}
    specs = [(c, c) for c in CONDITION_PREFIX_ORDER if c in present]
    extras = sorted(c for c in present if c not in CONDITION_PREFIX_ORDER)
...
MEAN_SEM_INDIV_COLOR = "0.72"
MEAN_SEM_MEAN_COLOR = "r"
CONDITION_MEAN_SEM_STYLE = {
...
def assign_binned_min_everymin(
    ...
    time_threshold: float = BIN_TIME_THRESHOLD_SEC,
    bin_percent_median: float = BIN_PERCENT_MEDIAN,
) -> pd.DataFrame:
...
def plot_spine_volume_mean_sem(
    ...
    *,
    ...
    indiv_color: str = MEAN_SEM_INDIV_COLOR,
) -> int:
    ...
    if plot_df.empty:
        return 0
    # One value per spine per bin so SEM is across spines, not frames.
    plot_df = (
...
def decorate_spine_volume_timecourse_ax(ax, ylim) -> None:
    """Uncaging bar, LTP window, y=0. X is binned_min."""
    ax.set_ylim(ylim)
    ax.set_xlim(BINNED_TIME_XLIM_MIN, BINNED_TIME_XLIM_MAX)
    ...
    ax.fill_between(
        np.array(LTP_data_point_after_min_between, dtype=float),
        ylim[0],
        ylim[1],
        color="pink",
        alpha=0.3,
...
acquisiton_start_datetime_str = "2026-09-09T16:30:00.000"
...
for each_group in fulltimeseries_df["group"].unique():
    for each_set_label in fulltimeseries_df[fulltimeseries_df["group"] == each_group]["set_label"].unique():
        ...
        each_condition = assign_condition(file_path=file_path_for_condition, group=each_group)
...
fulltimeseries_df = assign_binned_min_everymin(fulltimeseries_df)


# %% condition groups by AP5_ vs cnt_ filename prefix
condition_group_specs = build_condition_group_specs(fulltimeseries_df)
...
if unknown_ids:
    print(f"Warning: unmatched filename prefix (not AP5_ or cnt_): {unknown_ids}")
...
for each_header_name, each_condition in condition_group_specs:
    ...
    for each_uncaging_power_coherent_mW in sorted_uncaging_powers:
        ...
        decorate_spine_volume_timecourse_ax(ax, ylim_mean_sem)
...
for each_header_name, each_condition in build_condition_group_specs(summary_df):
