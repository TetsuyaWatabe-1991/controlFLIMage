...
ch_1or2 =2
# Group by cont/drug using acq_time vs drug_addition_datetime_str (not filename substring).
group_header_dict = {
    "cont": "cont",
    "drug": "drug",
}
...
acquisiton_start_datetime_str = "2026-05-20T11:00:00.000"
drug_addition_datetime_str = "2026-05-20T20:30:00.000"

df_save_path_1 = r"//RY-LAB-WS04/ImagingData/Tetsuya/20260520/auto1\combined_df_1.pkl"
out_csv_path = r"//RY-LAB-WS04/ImagingData/Tetsuya/20260520/auto1\combined_df_1_intensity_lifetime_all_frames.csv"
...
drug_addition_datetime = datetime.datetime.strptime(
    drug_addition_datetime_str, "%Y-%m-%dT%H:%M:%S.%f"
)
fulltimeseries_df["acq_time_datetime"] = pd.to_datetime(fulltimeseries_df["acq_time_str"])
fulltimeseries_df["cont_drug_group"] = np.where(
    fulltimeseries_df["acq_time_datetime"] < drug_addition_datetime,
    "cont",
    "drug",
)
...
summary_df["cont_drug_group"] = np.where(
    summary_df["acq_time_datetime"] < drug_addition_datetime,
    "cont",
    "drug",
)
...
for each_header, each_header_name in group_header_dict.items():
    eachgroup_df = fulltimeseries_df[fulltimeseries_df["cont_drug_group"] == each_header]
    ...
    for each_uncaging_power_coherent_mW in fulltimeseries_df["uncaging_power_coherent_mW"].unique():
        ...
        for each_plot_type, each_plot_info in plot_info_dict.items():
            ...
            if "xlim_max_cap" in each_plot_info:
                x0, x1 = plt.gca().get_xlim()
                plt.xlim(x0, min(each_plot_info["xlim_max_cap"], x1))
            # #plot mean with SEM
...
valid_group_headers = []
for each_header, each_header_name in group_header_dict.items():
    if not fulltimeseries_df[fulltimeseries_df["cont_drug_group"] == each_header].empty:
    ...
    for each_plot_type, each_plot_info in plot_info_dict.items():
        ...
        for col_idx, (each_header, each_header_name) in enumerate(valid_group_headers):
            ...
            for row_idx, each_uncaging_power_coherent_mW in enumerate(sorted_uncaging_powers):
                ...
                if "xlim_max_cap" in each_plot_info:
                    x0, x1 = ax.get_xlim()
                    ax.set_xlim(x0, min(each_plot_info["xlim_max_cap"], x1))
...
for each_header, each_header_name in group_header_dict.items():
    each_header_summary_df = summary_df[summary_df["cont_drug_group"] == each_header]
...
valid_group_headers = []
for each_header, each_header_name in group_header_dict.items():
    if not summary_df[summary_df["cont_drug_group"] == each_header].empty:
        valid_group_headers.append((each_header, each_header_name))
