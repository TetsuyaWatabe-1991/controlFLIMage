...
# 25-35 min spine delta F/F0 (swarmplot metric): exclude group_set_id if value exceeds this.
extreme_ltp_delta_ff0_threshold = 4
...
save_folder = os.path.join(os.path.dirname(df_save_path_1), "summary_exclude_extremeLTP")
...
extreme_ltp_col = f"delta_FF0_intensity_ch{ch_1or2}"
extreme_ltp_mask = summary_df[extreme_ltp_col].gt(extreme_ltp_delta_ff0_threshold)
extreme_ltp_ids = summary_df.loc[extreme_ltp_mask, "group_set_id"].tolist()
print(f"Excluded sets (25-35 min spine {extreme_ltp_col} > {extreme_ltp_delta_ff0_threshold}): {extreme_ltp_ids}")
summary_df = summary_df[~extreme_ltp_mask].reset_index(drop=True)
fulltimeseries_df = fulltimeseries_df[~fulltimeseries_df["group_set_id"].isin(extreme_ltp_ids)].reset_index(drop=True)
