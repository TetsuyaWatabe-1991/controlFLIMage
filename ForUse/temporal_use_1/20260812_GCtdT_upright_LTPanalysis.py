...
acquisiton_start_datetime_str = "2026-08-12T15:50:00.000"
# df_save_path_1 = r"G:/ImagingData/Tetsuya/20260515/auto1\combined_df_1.pkl"
df_save_path_1 = r"//RY-LAB-YAS15/Users/Yasudalab/Documents/Tetsuya_Imaging/20260812/auto2\combined_df_respan.pkl"
out_csv_path = r"//RY-LAB-YAS15/Users/Yasudalab/Documents/Tetsuya_Imaging/20260812/auto2\combined_df_respan_intensity_lifetime_all_frames.csv"
    ...
    for each_set_label in fulltimeseries_df[fulltimeseries_df["group"] == each_group]["set_label"].unique():
        ...
        for each_ROI_name in ROI_name_list:
            for each_ch in ['Ch1', 'Ch2']:
                ...
                each_phase = 'post'
                post_phase_df = each_df[
                    (each_df["phase"] == each_phase)
                    & (each_df["group_set_id"] == group_set_id)
                ]
                post_LTP_data_point_df, ltp_sel = select_ltp_post_frames(
                    post_phase_df,
                    time_col="aligned_time_sec",
                    window_min=LTP_data_point_after_min_between,
                )
                if each_ROI_name == ROI_name_list[0] and each_ch == "Ch1":
                    times_min = (
                        (post_LTP_data_point_df["aligned_time_sec"] / 60.0)
                        .round(2)
                        .tolist()
                        if len(post_LTP_data_point_df)
                        else []
                    )
                    print(
                        f"{group_set_id}: LTP post frames ({ltp_sel}) at {times_min} min"
                    )
                for each_signal in ['lifetime', 'intensity']:
...
_ts_for_ylim = fulltimeseries_df[
    fulltimeseries_df["aligned_time_sec"] < TIMESERIES_PLOT_MAX_SEC
]
full_range_ylim = [_ts_for_ylim[f"Spine_Ch{ch_1or2}_intensity_normalized"].min()-0.1, 
                _ts_for_ylim[f"Spine_Ch{ch_1or2}_intensity_normalized"].max()+0.1]
plot_info_dict = {
...
for each_header_name, header_list in group_header_combined.items():
    ...
    plot_df = eachgroup_df[eachgroup_df["aligned_time_sec"] < TIMESERIES_PLOT_MAX_SEC]
    ...
    for each_plot_type, each_plot_info in plot_info_dict.items():
        ...
        savepath = os.path.join(save_folder, f"{group_header_file_tag(header_list)}_{each_plot_type}_lineplot_time_series.png")
        plt.savefig(savepath, dpi=150, bbox_inches = "tight")
        plt.show()

# %% line plot (tiled panels for group_header (all uncaging powers pooled))
valid_group_headers = []
for each_header_name, header_list in group_header_combined.items():
...
for each_header_name, header_list in group_header_combined.items():
    ...
    for each_plot_type, each_plot_info in plot_info_dict.items():
        ...
        plt.gca().spines["top"].set_visible(False)
        plt.gca().spines["right"].set_visible(False)
        savepath = os.path.join(save_folder, f"{group_header_file_tag(header_list)}_{each_plot_type}_plot_swarmplot.png")
        plt.savefig(savepath, dpi=150, bbox_inches = "tight")
        plt.show()

# %% swarm plot (tiled panels for group_header (all uncaging powers pooled))
valid_group_headers = []
for each_header_name, header_list in group_header_combined.items():
...
if len(combined_rows) >= 2:
    ...
    if len(present_groups) >= 2:
        ...
        ax.set_xlabel("")
        ax.set_title("all powers", fontsize=10)
        ax.spines["top"].set_visible(False)
        ax.spines["right"].set_visible(False)
        ...
        savepath = os.path.join(
            save_folder,
            "panel_intensity_swarmplot_combined_stats_all_powers.png",
        )
        plt.savefig(savepath, dpi=300, bbox_inches="tight", facecolor="white")
        stats_csv = os.path.join(
            save_folder,
            "panel_intensity_swarmplot_combined_stats_all_powers.csv",
        )
        pd.DataFrame(
            [
                {
                    "uncaging_power_coherent_mW": "all",
                    "group_a": g0,
                    "group_b": g1,
                    "n_a": len(v0),
                    "n_b": len(v1),
...
for each_header_name, header_list in group_header_combined.items():
    ...
    plot_df = eachgroup_df[
        (eachgroup_df["phase"] == "unc")
        & (eachgroup_df["aligned_time_sec"] < TIMESERIES_PLOT_MAX_SEC)
    ]
    print(each_header_name, "all powers", len(plot_df))
...
# %% line plot, transient (tiled panels for group_header (all uncaging powers pooled))
valid_group_headers = []
for each_header_name, header_list in group_header_combined.items():
    if not df_matches_group_headers(fulltimeseries_df, header_list).empty:
        valid_group_headers.append((header_list, each_header_name))
...
for each_header_name, header_list in group_header_combined.items():
    ...
    for each_plot_type, each_plot_info in plot_info_dict.items():
        ...
        plt.gca().spines["top"].set_visible(False)
        plt.gca().spines["right"].set_visible(False)
        savepath = os.path.join(save_folder, f"{group_header_file_tag(header_list)}_{each_plot_type}_plot_transient_swarmplot.png")
        plt.savefig(savepath, dpi=150, bbox_inches = "tight")
        plt.show()

# %% swarm plot for transient (tiled panels for group_header (all uncaging powers pooled))
valid_group_headers = []
for each_header_name, header_list in group_header_combined.items():
    if not df_matches_group_headers(summary_df, header_list).empty:
        valid_group_headers.append((header_list, each_header_name))
...
for each_header_name, header_list in group_header_combined.items():
    ...
    for each_plot_type, each_plot_info in plot_info_dict.items():
        ...
        plt.gca().spines["top"].set_visible(False)
        plt.gca().spines["right"].set_visible(False)
        savepath = os.path.join(save_folder, f"{group_header_file_tag(header_list)}_{each_plot_type}_plot_scatterplot.png")
        plt.savefig(savepath, dpi=150, bbox_inches = "tight")
        plt.show()

# %% scatter plot (tiled panels for group_header (all uncaging powers pooled))
valid_group_headers = []
for each_header_name, header_list in group_header_combined.items():
    if not df_matches_group_headers(summary_df, header_list).empty:
        valid_group_headers.append((header_list, each_header_name))
...
# %% plot against acq_time_str (tiled panels for group_header (all uncaging powers pooled))
valid_group_headers = []
for each_header_name, header_list in group_header_combined.items():
    if not df_matches_group_headers(summary_df, header_list).empty:
        valid_group_headers.append((header_list, each_header_name))
