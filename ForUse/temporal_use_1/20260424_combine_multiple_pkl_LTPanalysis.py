...
# List of acquisitions to combine.
# Each entry has its own group_header_dict, start datetime, and file paths.
# Acquisitions whose group_header_dict values are the same will be plotted together.
...
acquisition_list = [
    {
        ...
        "out_csv_path": r"G:/ImagingData/Tetsuya/20260421/cont_and_tlnKO\combined_df_1_intensity_lifetime_all_frames.csv",
    },
    {
        "group_header_dict": {
            "": "control",
        },
        "acquisiton_start_datetime_str": "2026-04-21T11:00:00.000",
        ...
        "acquisiton_start_datetime_str": "2026-04-17T15:10:00.000",
        "df_save_path_1": r"G:\ImagingData\Tetsuya\20260417\TlnKO\auto1\combined_df_1.pkl",
        "out_csv_path": r"G:\ImagingData\Tetsuya\20260417\TlnKO\auto1\combined_df_1_intensity_lifetime_all_frames.csv",
    },        
        {
        "group_header_dict": {
            "": "control",
        },
        "acquisiton_start_datetime_str": "2026-04-16T16:10:00.000",
        "df_save_path_1": r"G:\ImagingData\Tetsuya\20260416\contpx458_1\combined_df_1.pkl",
        "out_csv_path": r"G:\ImagingData\Tetsuya\20260416\contpx458_1\combined_df_1_intensity_lifetime_all_frames.csv",
    },
        {
        "group_header_dict": {
            "": "control",
        },
        "acquisiton_start_datetime_str": "2026-04-16T19:10:00.000",
        "df_save_path_1": r"G:\ImagingData\Tetsuya\20260416\contpx458_2\combined_df_1.pkl",
        "out_csv_path": r"G:\ImagingData\Tetsuya\20260416\contpx458_2\combined_df_1_intensity_lifetime_all_frames.csv",
    },
]

# Merge all group_header_dicts; acquisitions with the same value will share plots
merged_group_header_dict = {}
for _acq in acquisition_list:
    for _header, _name in _acq["group_header_dict"].items():
        if _header not in merged_group_header_dict:
            merged_group_header_dict[_header] = _name

# Group headers by display name so that acquisitions sharing the same name
# are plotted in the same panel (e.g. "cont_" and "contKO_" both -> "control")
from collections import defaultdict
_name_to_headers: dict = defaultdict(list)
for _h, _n in merged_group_header_dict.items():
    _name_to_headers[_n].append(_h)
# plot_group_dict: {display_name: [header1, header2, ...]}
plot_group_dict: dict = dict(_name_to_headers)
...
# Save folder: combined_summary under the first acquisition's directory
save_folder = os.path.join(os.path.dirname(acquisition_list[0]["df_save_path_1"]), "combined_summary")
os.makedirs(save_folder, exist_ok=True)

#%% Load and process each acquisition, then combine into single dataframes
all_combined_df_list = []
all_fulltimeseries_df_list = []
all_summary_df_list = []

for acq_idx, acq in enumerate(acquisition_list):
    df_save_path_1 = acq["df_save_path_1"]
    out_csv_path = acq["out_csv_path"]
    acquisiton_start_datetime_str = acq["acquisiton_start_datetime_str"]
    ...
    for each_ROI_name in ROI_name_list:
        for each_ch in ['Ch1', 'Ch2']:
            fulltimeseries_df.loc[:, f"{each_ROI_name}_{each_ch}_intensity_div_by_nAve"] = \
                fulltimeseries_df.loc[:, f"{each_ROI_name}_{each_ch}_intensity"] / fulltimeseries_df.loc[:, "nAveFrame"]
    ...
    print(f"[acq {acq_idx}] uncaging power in combined_df:")
    ...
    # Build power % -> power mW dict from powermeter calibration JSON
    ...
    for each_group in fulltimeseries_df["group"].unique():
        for each_set_label in fulltimeseries_df[fulltimeseries_df["group"] == each_group]["set_label"].unique():
            # prefix acq_idx to avoid collision between acquisitions
            group_set_id = f"acq{acq_idx}_{each_group}_{each_set_label}"
            ...
            for each_ROI_name in ROI_name_list:
                for each_ch in ['Ch1', 'Ch2']:
                    ...
                    fulltimeseries_df.loc[each_df.index, f"{each_ROI_name}_{each_ch}_lifetime_normalized"] = \
                        fulltimeseries_df.loc[each_df.index, f"{each_ROI_name}_{each_ch}_lifetime"] - pre_phase_lifetime
                    pre_phase_intensity = each_df[each_df["phase"] == "pre"][f"{each_ROI_name}_{each_ch}_intensity_div_by_nAve"].mean()
                    fulltimeseries_df.loc[each_df.index, f"{each_ROI_name}_{each_ch}_intensity_normalized"] = \
                        fulltimeseries_df.loc[each_df.index, f"{each_ROI_name}_{each_ch}_intensity_div_by_nAve"] / pre_phase_intensity - 1
            ...
            each_summary_dict["acq_idx"] = acq_idx
            ...
            for _ch_n, _ch_col in [(1, "Ch1"), (2, "Ch2")]:
                _pre_mean = each_summary_dict[f"{_ch_col}_pre_intensity"]
                if len(first_post_df) > 0 and _pre_mean > 0:
                    each_summary_dict[f"first_post_FF0_intensity_ch{_ch_n}"] = first_post_df[f"Spine_{_ch_col}_intensity"].iloc[0] / _pre_mean - 1
                else:
                    each_summary_dict[f"first_post_FF0_intensity_ch{_ch_n}"] = np.nan
    ...
    fulltimeseries_df.loc[fulltimeseries_df["phase"] == "pre", "binned_time_sec"] = pre_phase_sec
    fulltimeseries_df.loc[fulltimeseries_df["phase"] == "post", "binned_time_sec"] = post_phase_sec
    ...
    for each_group_set_id in fulltimeseries_df["group_set_id"].unique():
        each_df = fulltimeseries_df[fulltimeseries_df["group_set_id"] == each_group_set_id]
        unc_df_local = each_df[each_df["phase"] == "unc"]
        uncaging_power_coherent_mW_list = list(unc_df_local["uncaging_power_coherent_mW"].unique())
        ...
        length_of_unc_df = len(unc_df_local)
    ...
    # add time_hours_incubation per acquisition before merging
    ...
    # Assign display_group_name to each row based on group_header_dict.
    # An empty string key "" means all groups in this acquisition belong to that display name.
    acq_group_header_dict = acq["group_header_dict"]
    def _assign_display_name(group_val: str) -> str:
        for _hdr, _nm in acq_group_header_dict.items():
            if _hdr == "" or _hdr in str(group_val):
                return _nm
        return ""
    fulltimeseries_df["display_group_name"] = fulltimeseries_df["group"].map(_assign_display_name)
    summary_df["display_group_name"] = summary_df["group"].map(_assign_display_name)

    all_combined_df_list.append(combined_df)
    all_fulltimeseries_df_list.append(fulltimeseries_df)
    all_summary_df_list.append(summary_df)

# Concatenate all acquisitions into single dataframes for plotting
combined_df = pd.concat(all_combined_df_list, ignore_index=True)
fulltimeseries_df = pd.concat(all_fulltimeseries_df_list, ignore_index=True)
summary_df = pd.concat(all_summary_df_list, ignore_index=True)

# Merge uncaging power values that are within 0.2 mW of each other into a single representative value.
# Sort unique powers, then greedily group consecutive values within 0.2 mW tolerance,
# replacing each group with its mean rounded to 1 decimal place.
_unc_pow_tolerance = 0.2
_raw_powers = sorted(
    v for v in fulltimeseries_df["uncaging_power_coherent_mW"].dropna().unique()
)
_power_remap: dict = {}
_group_start = 0
for _i in range(1, len(_raw_powers) + 1):
    if _i == len(_raw_powers) or (_raw_powers[_i] - _raw_powers[_group_start] > _unc_pow_tolerance):
        _group_vals = _raw_powers[_group_start:_i]
        _rep = round(float(np.mean(_group_vals)), 1)
        for _v in _group_vals:
            _power_remap[_v] = _rep
        _group_start = _i
print("Uncaging power remapping:", _power_remap)

for _df in [fulltimeseries_df, summary_df, combined_df]:
    _df["uncaging_power_coherent_mW"] = _df["uncaging_power_coherent_mW"].map(
        lambda v: _power_remap.get(v, v) if pd.notna(v) else v
    )
...
for each_header_name in plot_group_dict:
    eachgroup_df = fulltimeseries_df[fulltimeseries_df["display_group_name"] == each_header_name]
    ...
    for each_uncaging_power_coherent_mW in fulltimeseries_df["uncaging_power_coherent_mW"].unique():
        ...
        for each_plot_type, each_plot_info in plot_info_dict.items():
            ...
            savepath = os.path.join(save_folder, f"{each_header_name}_{each_plot_type}_{each_uncaging_power_coherent_mW}mW_lineplot_time_series.png")
...
valid_group_headers = []
for each_header_name in plot_group_dict:
    if not fulltimeseries_df[fulltimeseries_df["display_group_name"] == each_header_name].empty:
        valid_group_headers.append(each_header_name)
...
if len(sorted_uncaging_powers) > 0 and len(valid_group_headers) > 0:
    for each_plot_type, each_plot_info in plot_info_dict.items():
        ...
        for col_idx, each_header_name in enumerate(valid_group_headers):
            eachgroup_df = fulltimeseries_df[fulltimeseries_df["display_group_name"] == each_header_name]
...
for each_header_name in plot_group_dict:
    each_header_summary_df = summary_df[summary_df["display_group_name"] == each_header_name]
    ...
    for each_uncaging_power_coherent_mW in fulltimeseries_df["uncaging_power_coherent_mW"].unique():
        ...
        for each_plot_type, each_plot_info in plot_info_dict.items():
            ...
            plt.gca().spines["top"].set_visible(False)
            plt.gca().spines["right"].set_visible(False)
            savepath = os.path.join(save_folder, f"{each_header_name}_{each_plot_type}_{each_uncaging_power_coherent_mW}mW_plot_swarmplot.png")
            plt.savefig(savepath, dpi=150, bbox_inches = "tight")
            plt.show()
...
valid_group_headers = []
for each_header_name in plot_group_dict:
    if not summary_df[summary_df["display_group_name"] == each_header_name].empty:
        valid_group_headers.append(each_header_name)
...
for each_header_name in plot_group_dict:
    ...
    for each_uncaging_power_coherent_mW in fulltimeseries_df["uncaging_power_coherent_mW"].unique():
        ...
        for each_plot_type, each_plot_info in plot_info_dict.items():
            ...
            plt.gca().spines["top"].set_visible(False)
            plt.gca().spines["right"].set_visible(False)
            savepath = os.path.join(save_folder, f"{each_header_name}_{each_plot_type}_{each_uncaging_power_coherent_mW}mW_plot_scatterplot.png")
            plt.savefig(savepath, dpi=150, bbox_inches = "tight")
            plt.show()
...
# time_hours_incubation is already computed per-acquisition inside the loading loop
...
for each_header_name in plot_group_dict:
    group_summary = summary_df[summary_df["display_group_name"] == each_header_name]
    for each_pow in sorted(summary_df["uncaging_power_coherent_mW"].dropna().unique()):
        n = len(group_summary[group_summary["uncaging_power_coherent_mW"] == each_pow])
        if n > 0:
            print(f"  {each_header_name}, {each_pow} mW : n = {n}")
print("---------------------------------------")
