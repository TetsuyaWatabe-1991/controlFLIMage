...
from flim_summarize_func import (
    ...
    build_group_header_combined,
    df_matches_group_headers,
    group_header_file_tag,
    reshape_axes_to_2d,
)
...
acquisiton_start_datetime_str = "2026-07-01T14:30:00.000"
# df_save_path_1 = r"G:/ImagingData/Tetsuya/20260515/auto1\combined_df_1.pkl"
df_save_path_1 = r"G:/ImagingData/Tetsuya/20260701/auto1\combined_df_respan.pkl"
out_csv_path = r"G:/ImagingData/Tetsuya/20260701/auto1\combined_df_respan_intensity_lifetime_all_frames.csv"
...
for each_header_name, header_list in group_header_combined.items():
    eachgroup_df = df_matches_group_headers(fulltimeseries_df, header_list)
    ...
    for each_uncaging_power_coherent_mW in fulltimeseries_df["uncaging_power_coherent_mW"].unique():
        ...
        for each_plot_type, each_plot_info in plot_info_dict.items():
            ...
            savepath = os.path.join(save_folder, f"{group_header_file_tag(header_list)}_{each_plot_type}_{each_uncaging_power_coherent_mW}mW_lineplot_time_series.png")
...
valid_group_headers = []
for each_header_name, header_list in group_header_combined.items():
    if not df_matches_group_headers(fulltimeseries_df, header_list).empty:
        valid_group_headers.append((header_list, each_header_name))
...
if len(sorted_uncaging_powers) > 0 and len(valid_group_headers) > 0:
    for each_plot_type, each_plot_info in plot_info_dict.items():
        ...
        for col_idx, (header_list, each_header_name) in enumerate(valid_group_headers):
            eachgroup_df = df_matches_group_headers(fulltimeseries_df, header_list)
...
for each_header_name, header_list in group_header_combined.items():
    each_header_summary_df = df_matches_group_headers(summary_df, header_list)
    ...
    for each_uncaging_power_coherent_mW in fulltimeseries_df["uncaging_power_coherent_mW"].unique():
        ...
        for each_plot_type, each_plot_info in plot_info_dict.items():
            ...
            plt.gca().spines["top"].set_visible(False)
            plt.gca().spines["right"].set_visible(False)
            savepath = os.path.join(save_folder, f"{group_header_file_tag(header_list)}_{each_plot_type}_{each_uncaging_power_coherent_mW}mW_plot_swarmplot.png")
            plt.savefig(savepath, dpi=150, bbox_inches = "tight")
            plt.show()
...
valid_group_headers = []
for each_header_name, header_list in group_header_combined.items():
    if not df_matches_group_headers(summary_df, header_list).empty:
        valid_group_headers.append((header_list, each_header_name))
...
for each_header_name, header_list in group_header_combined.items():
    ...
    for each_uncaging_power_coherent_mW in fulltimeseries_df["uncaging_power_coherent_mW"].unique():
        ...
        for each_plot_type, each_plot_info in plot_info_dict.items():
            ...
            plt.gca().spines["top"].set_visible(False)
            plt.gca().spines["right"].set_visible(False)
            savepath = os.path.join(save_folder, f"{group_header_file_tag(header_list)}_{each_plot_type}_{each_uncaging_power_coherent_mW}mW_plot_transient_swarmplot.png")
            plt.savefig(savepath, dpi=150, bbox_inches = "tight")
            plt.show()
...
correlation_scatter_plot_keys = [
    "GCaMP_LTP_level_against_vs_DendriticShaft_F_F0",
    "GCaMP_LTP_level_against_vs_Spine_F_F0",
    f"delta_FF0_ch{ch_1or2}_vs_first_post_FF0_ch{ch_1or2}",
]
correlation_scatter_plot_info_dict = {
    k: plot_info_dict[k] for k in correlation_scatter_plot_keys
}

sorted_uncaging_powers_for_color = sorted(
    summary_df["uncaging_power_coherent_mW"].dropna().unique()
)
uncaging_power_palette = sns.color_palette(
    "tab10", n_colors=max(len(sorted_uncaging_powers_for_color), 1)
)
uncaging_power_color_dict = {
    p: uncaging_power_palette[i]
    for i, p in enumerate(sorted_uncaging_powers_for_color)
}


def _compute_pearson_corr(plot_df, x_col, y_col):
    valid_df = plot_df[[x_col, y_col]].dropna()
    n = len(valid_df)
    if n < 2:
        return np.nan, np.nan, n
    r, p = pearsonr(valid_df[x_col], valid_df[y_col])
    return r, p, n


def _format_corr_text(r, p, n):
    if np.isnan(r):
        return f"r = nan (n={n})"
    if p < 0.001:
        p_str = "p < 0.001"
        ...
        p_str = f"p = {p:.3f}"
    return f"r = {r:.2f}, {p_str} (n={n})"


def _annotate_corr_on_ax(ax, r, p, n):
    ax.text(
        0.05,
        0.95,
        _format_corr_text(r, p, n),
...
def _draw_linear_fit_on_ax(ax, plot_df, x_col, y_col, xlim):
    valid_df = plot_df[[x_col, y_col]].dropna()
    if len(valid_df) < 2:
        return
...
def _draw_corr_scatter_on_ax(ax, plot_df, each_plot_info, annotate_corr=True):
    sns.scatterplot(
        x=each_plot_info["x"],
        y=each_plot_info["y"],
        data=plot_df,
    ...
    if each_plot_info.get("plot_zero_lines"):
        ax.plot(
            [each_plot_info["xlim"][0], each_plot_info["xlim"][1]],
            [0, 0],
            "--",
            color="gray",
            linewidth=0.5,
    ...
    _draw_linear_fit_on_ax(
        ax, plot_df, each_plot_info["x"], each_plot_info["y"], each_plot_info["xlim"]
    )
    if annotate_corr:
        r, p, n = _compute_pearson_corr(
            plot_df, each_plot_info["x"], each_plot_info["y"]
        )
        _annotate_corr_on_ax(ax, r, p, n)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)


def _draw_corr_scatter_combined_on_ax(ax, plot_df, each_plot_info):
    sns.scatterplot(
        x=each_plot_info["x"],
        y=each_plot_info["y"],
        data=plot_df,
        hue="uncaging_power_coherent_mW",
        palette=uncaging_power_color_dict,
        hue_order=sorted_uncaging_powers_for_color,
        ax=ax,
    )
    ax.set_xlim(each_plot_info["xlim"])
    ax.set_ylim(each_plot_info["ylim"])
...
for each_header_name, header_list in group_header_combined.items():
    ...
    for each_uncaging_power_coherent_mW in fulltimeseries_df["uncaging_power_coherent_mW"].unique():
        ...
        for each_plot_type, each_plot_info in plot_info_dict.items():
            ...
            plt.gca().spines["top"].set_visible(False)
            plt.gca().spines["right"].set_visible(False)
            savepath = os.path.join(save_folder, f"{group_header_file_tag(header_list)}_{each_plot_type}_{each_uncaging_power_coherent_mW}mW_plot_scatterplot.png")
            plt.savefig(savepath, dpi=150, bbox_inches = "tight")
            plt.show()
...
# %% scatter plot with correlation (per uncaging power)
for each_header_name, header_list in group_header_combined.items():
    each_header_summary_df = df_matches_group_headers(summary_df, header_list)
    if len(each_header_summary_df) == 0:
        continue
    for each_uncaging_power_coherent_mW in fulltimeseries_df["uncaging_power_coherent_mW"].unique():
        each_group_same_unc_pow_df = each_header_summary_df[
            each_header_summary_df["uncaging_power_coherent_mW"] == each_uncaging_power_coherent_mW
        ]
        plot_df = each_group_same_unc_pow_df
        for each_plot_type, each_plot_info in correlation_scatter_plot_info_dict.items():
            fig, ax = plt.subplots(figsize=(3, 3))
            _draw_corr_scatter_on_ax(ax, plot_df, each_plot_info)
            ax.set_ylabel(each_plot_info["ylabel"])
            ax.set_xlabel(each_plot_info["xlabel"])
            ax.set_title(each_header_name + f", uncaging {each_uncaging_power_coherent_mW} mW")
            savepath = os.path.join(
                save_folder,
                f"{group_header_file_tag(header_list)}_{each_plot_type}_{each_uncaging_power_coherent_mW}mW_plot_scatterplot_with_corr.png",
            )
            plt.savefig(savepath, dpi=150, bbox_inches="tight")
            plt.show()

# %% scatter plot with correlation (per uncaging power, tiled panels)
sorted_uncaging_powers = sorted(fulltimeseries_df["uncaging_power_coherent_mW"].dropna().unique())
valid_group_headers = []
for each_header_name, header_list in group_header_combined.items():
    if not df_matches_group_headers(summary_df, header_list).empty:
...
if len(sorted_uncaging_powers) > 0 and len(valid_group_headers) > 0:
    for each_plot_type, each_plot_info in correlation_scatter_plot_info_dict.items():
        ...
        fig.subplots_adjust(left=0.24, right=0.98, top=0.9, bottom=0.16)
        savepath = os.path.join(save_folder, f"panel_{each_plot_type}_scatterplot_with_corr.png")
        plt.savefig(savepath, dpi=150, bbox_inches="tight")
        plt.show()

# %% scatter plot with correlation (combined across uncaging powers)
for each_header_name, header_list in group_header_combined.items():
    each_header_summary_df = df_matches_group_headers(summary_df, header_list)
    if len(each_header_summary_df) == 0:
        continue
    ...
    for each_plot_type, each_plot_info in correlation_scatter_plot_info_dict.items():
        ...
        _draw_corr_scatter_combined_on_ax(ax, plot_df, each_plot_info)
        ax.set_ylabel(each_plot_info["ylabel"])
        ax.set_xlabel(each_plot_info["xlabel"])
        ax.set_title(each_header_name + ", all uncaging powers")
        ax.legend(title="Uncaging (mW)", bbox_to_anchor=(1.02, 1), loc="upper left", fontsize=8)
        savepath = os.path.join(
            save_folder,
            f"{group_header_file_tag(header_list)}_{each_plot_type}_plot_scatterplot_with_corr_combined.png",
        )
        plt.savefig(savepath, dpi=150, bbox_inches="tight")
        plt.show()

# %% scatter plot with correlation (combined across uncaging powers, tiled panels)
if len(valid_group_headers) > 0:
    for each_plot_type, each_plot_info in correlation_scatter_plot_info_dict.items():
        n_cols = len(valid_group_headers)
        fig, axes = plt.subplots(
            1,
        ...
        for col_idx, (header_list, each_header_name) in enumerate(valid_group_headers):
            ...
            _draw_corr_scatter_combined_on_ax(ax, each_header_summary_df, each_plot_info)
            ax.set_xlabel(each_plot_info["xlabel"])
            if col_idx == 0:
                ax.set_ylabel(each_plot_info["ylabel"])
            else:
        ...
        fig.subplots_adjust(left=0.12, right=0.88, top=0.9, bottom=0.16)
        savepath = os.path.join(
            save_folder, f"panel_{each_plot_type}_scatterplot_with_corr_combined.png"
        )
        plt.savefig(savepath, dpi=150, bbox_inches="tight")
        plt.show()
...
incubation_split_hours = 10
incubation_bin_order = [f"< {incubation_split_hours} h", f">= {incubation_split_hours} h"]
summary_df["incubation_bin"] = np.where(
    summary_df["time_hours_incubation"] < incubation_split_hours,
    incubation_bin_order[0],
    incubation_bin_order[1],
)
summary_df["incubation_bin"] = pd.Categorical(
    summary_df["incubation_bin"], categories=incubation_bin_order, ordered=True
)
...
# %% swarm plot by incubation time (~10 h split)
def _ylim_with_margin(series):
    vmin, vmax = series.min(), series.max()
    margin = (vmax - vmin) * 0.1 if vmax > vmin else 0.1
    return [vmin - margin, vmax + margin]


incubation_swarm_plot_info_dict = {
    "spine_volume": {
        "ylabel": r"$\Delta$spine volume (a.u.)",
        "y": f"delta_FF0_intensity_ch{ch_1or2}",
    },
    "GCaMP_spine": {
        "ylabel": r"GCaMP F/F0 (spine)",
        "y": "transient_Spine_Ch1_intensity",
    },
    "GCaMP_shaft": {
        "ylabel": r"GCaMP F/F0 (shaft)",
        "y": "transient_DendriticShaft_Ch1_intensity",
    },
}
for each_plot_info in incubation_swarm_plot_info_dict.values():
    each_plot_info["ylim"] = _ylim_with_margin(summary_df[each_plot_info["y"]].dropna())
...
for each_header_name, header_list in group_header_combined.items():
    ...
    for each_uncaging_power_coherent_mW in fulltimeseries_df["uncaging_power_coherent_mW"].unique():
        ...
        plot_df = each_group_same_unc_pow_df
        for each_plot_type, each_plot_info in incubation_swarm_plot_info_dict.items():
            plt.figure(figsize=(3, 3))
            p = sns.swarmplot(
                x="incubation_bin",
                y=each_plot_info["y"],
                data=plot_df,
                order=incubation_bin_order,
                palette="tab10",
            )
            sns.boxplot(
                showmeans=True,
            ...
            for bin_idx, each_bin in enumerate(incubation_bin_order):
                bin_df = plot_df[plot_df["incubation_bin"] == each_bin]
                if bin_df.empty:
                    continue
                mean = bin_df[each_plot_info["y"]].mean()
                std = bin_df[each_plot_info["y"]].std()
                plt.text(bin_idx, mean, f"{mean:.2f} ± {std:.2f}", ha="center", va="bottom", fontsize=8)
            plt.ylabel(each_plot_info["ylabel"])
            ...
            savepath = os.path.join(
                save_folder,
                f"{group_header_file_tag(header_list)}_{each_plot_type}_incubation_split_{each_uncaging_power_coherent_mW}mW_swarmplot.png",
            )
            plt.savefig(savepath, dpi=150, bbox_inches="tight")
            plt.show()

# %% swarm plot by incubation time (~10 h split, tiled panels)
sorted_uncaging_powers = sorted(fulltimeseries_df["uncaging_power_coherent_mW"].dropna().unique())
valid_group_headers = []
for each_header_name, header_list in group_header_combined.items():
    if not df_matches_group_headers(summary_df, header_list).empty:
...
if len(sorted_uncaging_powers) > 0 and len(valid_group_headers) > 0:
    for each_plot_type, each_plot_info in incubation_swarm_plot_info_dict.items():
        ...
        for col_idx, (header_list, each_header_name) in enumerate(valid_group_headers):
            ...
            for row_idx, each_uncaging_power_coherent_mW in enumerate(sorted_uncaging_powers):
                ...
                for bin_idx, each_bin in enumerate(incubation_bin_order):
                    ...
                    mean = bin_df[each_plot_info["y"]].mean()
                    std = bin_df[each_plot_info["y"]].std()
                    ax.text(bin_idx, mean, f"{mean:.2f} ± {std:.2f}", ha="center", va="bottom", fontsize=7)
        ...
        fig.subplots_adjust(left=0.26, right=0.98, top=0.9, bottom=0.14)
        savepath = os.path.join(save_folder, f"panel_{each_plot_type}_incubation_split_swarmplot.png")
        plt.savefig(savepath, dpi=150, bbox_inches="tight")
        plt.show()
