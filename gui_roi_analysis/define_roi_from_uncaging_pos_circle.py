...
simple_roi_savefolder = r"C:\Users\WatabeT\Desktop\20250701\auto1\simple_roi_from_uncaging_pos"
...
for filepath_without_number in combined_df["filepath_without_number"].unique():
    ...
    # #for debug
    # if "lowmag2__highmag_2" not in filepath_without_number:
    #     continue


    #glob and sort by creation time
    ...
    # # 数値部分でソート
    # sorted_list = sorted(ini_path_list, key=lambda x: int(re.search(r'_(\d+)\.ini$', x).group(1)))
    ...
    for each_nth_set_label in each_filepath_df["nth_set_label"].unique():
        ...
        each_set_include_unc_df = combined_df[(combined_df["nth_set_label"] == each_nth_set_label)
                                    & (combined_df["filepath_without_number"] == filepath_without_number)
                                    # & (combined_df["nth_omit_induction"] > 0)
                                    ]
        
        if len(each_set_include_unc_df) < 3:
            continue

        unc_df = each_set_include_unc_df[each_set_include_unc_df["phase"] == "unc"]
        unc_x = unc_df["center_x"].iloc[0]
        unc_y = unc_df["center_y"].iloc[0]

        drift_pre_last_unc_df = each_set_include_unc_df[each_set_include_unc_df["phase"] == 'pre'].iloc[-1]
        drift_pre_last_unc_df_idx = each_set_include_unc_df[each_set_include_unc_df["phase"] == 'pre'].index[-1]


        each_set_df = each_set_include_unc_df[each_set_include_unc_df["nth_omit_induction"] > 0]

        spine_inipath = non_rejected_inipath[int(each_nth_set_label)]
        ...
        for idx in each_set_df.index:
            ...
            shift_x_before_unc = drift_pre_last_unc_df["shift_x"] + drift_pre_last_unc_df["small_shift_x"]
            shift_y_before_unc = drift_pre_last_unc_df["shift_y"] + drift_pre_last_unc_df["small_shift_y"]
            
            shifted_unc_y = unc_y + (each_set_df.loc[idx, "shift_y"] + each_set_df.loc[idx, "small_shift_y"])*SHIFT_DIRECTION - shift_y_before_unc*SHIFT_DIRECTION
            shifted_unc_x = unc_x + (each_set_df.loc[idx, "shift_x"] + each_set_df.loc[idx, "small_shift_x"])*SHIFT_DIRECTION - shift_x_before_unc*SHIFT_DIRECTION
            # shifted_dend_intercept = dend_intercept - dend_slope * (each_set_df.loc[idx, "shift_x"] + each_set_df.loc[idx, "small_shift_x"])*SHIFT_DIRECTION + (each_set_df.loc[idx, "shift_y"] + each_set_df.loc[idx, "small_shift_y"])*SHIFT_DIRECTION - shift_y_before_unc*SHIFT_DIRECTION*(-1)
            # shifted_spine_y = unc_y 
            # shifted_spine_x = unc_x
            ...
            if idx == each_set_df.index[0]:
                nearest_to_unc_y, nearest_to_unc_x = calc_point_on_line_close_to_xy(x = shifted_unc_x, y = shifted_unc_y, 
                                            slope = dend_slope, 
                                            intercept = shifted_dend_intercept)

                distance_between_unc_and_dend = np.sqrt((nearest_to_unc_x - shifted_unc_x)**2 + (nearest_to_unc_y - shifted_unc_y)**2)
                circle_radius = distance_between_unc_and_dend - (rect_height/2 + 1)


            rr_circ, cc_circ = disk((shifted_unc_y, shifted_unc_x), circle_radius, shape=z_proj.shape)
            ...
            plt.scatter(shifted_spine_x, shifted_spine_y, s=5, marker=".", color="red")
            if idx == drift_pre_last_unc_df_idx:
                plt.scatter(unc_x, unc_y, s=10, marker="x", color="cyan")
