    ...
    for each_filepath_without_number in combined_df["filepath_without_number"].unique():
        ...
        for each_group in each_filegroup_df["group"].unique():
            ...
            for each_set_label in each_group_df["nth_set_label"].unique():
                ...
                # Per-frame source CSV for easy verification (test use)
                unc_basename = os.path.basename(unc_path)
                frame_info_rows = []
                for i in range(pre_stack.shape[0]):
                    frame_info_rows.append({"Frame": i, "Z_projection": True, "nth_slice": "", "filename": pre_filenames[i] if i < len(pre_filenames) else "", "phase": "pre"})
                for k in range(unc_stack.shape[0]):
                    frame_info_rows.append({"Frame": pre_stack.shape[0] + k, "Z_projection": False, "nth_slice": k, "filename": unc_basename, "phase": "uncaging"})
                for i in range(post_stack.shape[0]):
                    frame_info_rows.append({"Frame": pre_stack.shape[0] + unc_stack.shape[0] + i, "Z_projection": True, "nth_slice": "", "filename": post_filenames[i] if i < len(post_filenames) else "", "phase": "post"})
    ...
    for each_filepath_without_number in combined_df["filepath_without_number"].unique():
        ...
        for each_group in each_filegroup_df["group"].unique():
            ...
            for each_set_label in each_group_df["nth_set_label"].unique():
                ...
                def _row_from_file(file_path: str, frame_idx: int, stack_frame_idx: int, phase: str, time_sec: float):
                    iminfo = FileReader()
                    iminfo.read_imageFile(file_path, True)
                    imagearray = np.array(iminfo.image)
                    return _row_from_flim_data(
                        imagearray, iminfo, file_path, frame_idx, stack_frame_idx, phase, time_sec,
                    ...
                    if os.path.exists(unc_path):
                        ...
                        for t in range(T):
                            ...
                            all_rows.append(_row_from_flim_data(
                                imagearray_unc, iminfo_unc, unc_path, t, n_pre + t, "unc", time_sec,
                                roi_raw, corrected_uncaging_z, z_plus_minus, each_set_label, each_group,
                                fitter, sync_rate, photon_threshold, total_photon_threshold,
                            ))
                            processed += 1
