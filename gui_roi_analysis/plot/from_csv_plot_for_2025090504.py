...
combined_df_csv_path = r"G:\ImagingData\Tetsuya\20250904\auto1\summary\combined_df_after_analysis.csv"
LTP_point_df_csv_path = r"G:\ImagingData\Tetsuya\20250904\auto1\summary\LTP_point_df_after_analysis.csv"
...
display('path_stem_set', path_stem_set)
...
## GCaMP Spine F/F0 against norm_intensity
plt.figure(figsize = (4, 3))
p = sns.swarmplot(x="condition", y="GCaMP_Spine_F_F0", data=LTP_point_df)
...
plt.gca().spines['right'].set_visible(False)
plt.gca().spines['top'].set_visible(False)
plt.savefig(os.path.join(save_plot_folder, "GCaMP_Spine_F_F0_against_condition.png"), dpi=150,bbox_inches="tight")
plt.show()



## GCaMP dendritic shaft F/F0 against norm_intensity
plt.figure(figsize = (4, 3))
p = sns.swarmplot(x="condition", y="GCaMP_DendriticShaft_F_F0", data=LTP_point_df)
...
plt.gca().spines['right'].set_visible(False)
plt.gca().spines['top'].set_visible(False)
plt.savefig(os.path.join(save_plot_folder, "GCaMP_DendriticShaft_F_F0_against_condition.png"), dpi=150,bbox_inches="tight")
plt.show()
...
for each_group in LTP_point_df["condition"].unique():
    ...
    plt.figure(figsize=(4, 3))
    plt.title(each_group)
    sns.scatterplot(x='GCaMP_Spine_F_F0', y='norm_intensity', data=each_group_df)
    plt.xlim([0,15])
    plt.ylim([-0.5,4.5])
    plt.ylabel("Normalized $\Delta$volume (a.u.)")
    plt.xlabel("GCaMP Spine F/F0")
    plt.savefig(os.path.join(save_plot_folder, f"deltavolume_against_GCaMP_Spine_F_F0_{each_group}.png"), dpi=150,bbox_inches="tight")
    plt.show()

    plt.figure(figsize=(4, 3))
    plt.title(each_group)
    sns.scatterplot(x='GCaMP_DendriticShaft_F_F0', y='norm_intensity', data=each_group_df)
    plt.xlim([0,15])
    plt.ylim([-0.5,4.5])
    plt.ylabel("Normalized $\Delta$volume (a.u.)")
    plt.xlabel("GCaMP Dendritic Shaft F/F0")
    plt.savefig(os.path.join(save_plot_folder, f"deltavolume_against_GCaMP_DendriticShaft_F_F0_{each_group}.png"), dpi=150,bbox_inches="tight")
    plt.show()
