# %%
"""
TIFF-only ROI saving version of example_gui_usage_reduce_asking.py
This script saves ROI masks to TIFF files only, without saving to dataframe.
"""
...
from gui_integration_tiff_only import load_roi_masks_from_tiff_and_create_shifted_masks
...
from plot_timestamp_utils import save_plot_timestamp_to_ini, should_regenerate_group_plots, print_simple_summary
...
if (Do_without_asking == True) or (yn1 == True):  
    ...
    #save error_dict_combined to csv
    error_dict_combined_df = pd.DataFrame(error_dict_combined.items(), columns=["set_label", "error_message"])
    error_dict_combined_df.to_csv(os.path.join(os.path.dirname(df_save_path_1), "error_dict_combined.csv"), index=False)
...
# %%
# Launch the new file selection GUI with TIFF-only mode
print("="*60)
print("TIFF-ONLY MODE: ROI masks will be saved to TIFF files, not dataframe")
print("="*60)
if (Do_without_asking and skip_gui_TF) == False:
    ...
    app.exec_()
    print("ROI visualization plots completed (TIFF-only mode).")
    # Note: In TIFF-only mode, we don't reload from dataframe since ROI masks aren't saved there
    combined_df = pd.read_pickle(df_save_path_1)
...
# NOTE: In TIFF-only mode, we need to load ROI masks from TIFF files
print("\n" + "="*40)
print("Creating small ROI visualization plots (TIFF-only mode)...")
print("NOTE: Loading ROI masks from TIFF files instead of dataframe")
...
if yn2:
    ...
    for each_filepath in combined_df['filepath_without_number'].unique():
        ...
        for each_set_label in each_group_df["nth_set_label"].unique():
            ...
            # Get TIFF directory and basename for ROI mask files
            tiff_dir = os.path.dirname(temp_tiff_path)
            tiff_basename = os.path.basename(temp_tiff_path)
            ...
            for nth_omit_induction in each_label_df["relative_nth_omit_induction"].unique():
                ...
                # Load ROI masks from TIFF files instead of dataframe
                each_spine_roi_mask = None
                each_dendritic_shaft_roi_mask = None
                each_background_roi_mask = None
                
                frame_idx = int(nth_omit_induction)
                ...
                for roi_type in roi_types:
                    ...
                    if os.path.exists(roi_tiff_path):
                        ...
                        if frame_idx < roi_stack.shape[0]:
                            ...
                            if roi_type == "Spine":
                                each_spine_roi_mask = roi_mask
                            elif roi_type == "DendriticShaft":
                                each_dendritic_shaft_roi_mask = roi_mask
                            elif roi_type == "Background":
                                each_background_roi_mask = roi_mask
                            print(f"    Loaded {roi_type} ROI mask from: {roi_tiff_path}")
                    else:
                        print(f"    Warning: ROI mask TIFF not found: {roi_tiff_path}")
                ...
                if not any_roi_defined:
                    print(f"    Skipping Set: {each_set_label}, Frame: {nth_omit_induction} - No ROI masks found in TIFF files")
                    continue
                ...
                plt.title(f"Group: {display_group}, Set: {each_set_label}, Frame: {nth_omit_induction} (TIFF-only mode)")
    ...
    if Do_without_asking == False:
        send_slack_url_default(message = "finished saving small image and ROI mask plots with INI timestamp checking (TIFF-only mode).")

# %%
# Load ROI masks from TIFF files and create shifted_mask
print("\n" + "="*40)
print("Loading ROI masks from TIFF files and creating shifted_mask...")
print("="*40)
...
for each_filepath in combined_df['filepath_without_number'].unique():
    ...
    for each_set_label in each_group_df["nth_set_label"].unique():
        ...
        # Find ROI mask TIFF files
        tiff_path_dict = {}
        for roi_type in roi_types:
            roi_tiff_path = os.path.join(tiff_dir, f"{tiff_basename_no_ext}_{roi_type}_roi_mask.tif")
            if os.path.exists(roi_tiff_path):
                tiff_path_dict[roi_type] = roi_tiff_path
                print(f"Found {roi_type} ROI mask: {roi_tiff_path}")
            else:
                print(f"Warning: {roi_type} ROI mask not found: {roi_tiff_path}")
        
        if tiff_path_dict:
            # Load ROI masks from TIFF and create shifted_mask
            combined_df = load_roi_masks_from_tiff_and_create_shifted_masks(
                combined_df, tiff_path_dict, display_group, each_set_label,
...
print("\n" + "="*40)
print("Creating full-size ROI visualization plots (TIFF-only mode)...")
...
if yn3:
    ...
    # Note: In TIFF-only mode, we check for shifted_mask instead of roi_mask
    filelist = combined_df[(combined_df["nth_omit_induction"] != -1) &
                            (combined_df["nth_set_label"] != -1) &
                            (combined_df["Spine_shifted_mask"].notna())
                            ]["file_path"].tolist()
    ...
    print("All processing completed successfully (TIFF-only mode)!")
    # send_slack_url_default(message = "finished saving fullsize image and ROI mask plots with INI timestamp checking (TIFF-only mode).")
    ...
    for each_filepath in combined_df["filepath_without_number"].unique():
        ...
        for each_set_label in each_group_df["nth_set_label"].unique():
            ...
            for each_fluor_ch_1or2 in Fluor_ch_1or2_dict.keys():
                ...
                # Note: In TIFF-only mode, we need to calculate roi_area_pixels from shifted_mask
                spine_mask = combined_df.loc[nth_omit_induction_before_unc, "Spine_shifted_mask"]
                spine_area = spine_mask.sum() if isinstance(spine_mask, np.ndarray) else 0
                dendritic_shaft_mask = combined_df.loc[nth_omit_induction_before_unc, "DendriticShaft_shifted_mask"]
                dendritic_shaft_area = dendritic_shaft_mask.sum() if isinstance(dendritic_shaft_mask, np.ndarray) else 0
                background_mask = combined_df.loc[nth_omit_induction_before_unc, "Background_shifted_mask"]
                background_area = background_mask.sum() if isinstance(background_mask, np.ndarray) else 0
                
                spine_subBG = combined_df.loc[uncaging_df_index, f"{each_fluor_ch_1or2}_Spine_array"]/spine_area - combined_df.loc[uncaging_df_index, f"{each_fluor_ch_1or2}_Background_array"]/background_area
                dendritic_shaft_subBG = combined_df.loc[uncaging_df_index, f"{each_fluor_ch_1or2}_DendriticShaft_array"]/dendritic_shaft_area - combined_df.loc[uncaging_df_index, f"{each_fluor_ch_1or2}_Background_array"]/background_area
